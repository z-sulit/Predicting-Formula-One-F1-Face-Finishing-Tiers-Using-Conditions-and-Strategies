...
The non-significance of the tire-compound dummies (Tire\_SOFT and
Tire\_MEDIUM for Podium) suggests that \textit{which} compound a driver uses
matters less than \textit{how} the tire degrades over its stint. This
supports the theoretical position that degradation dynamics, rather than
static compound labels, drive performance differentiation.
...
\textcite{varoquaux2019} and \textcite{wong2023}, who showed that
group-aware validation produces more conservative and realistic performance
estimates. Our 62.36\% accuracy is therefore best read as an honest,
out-of-sample estimate for grouped racing data rather than an inflated score.
...
\begin{enumerate}
  ...
  \item \textbf{DNF protocol limitation.} Reassigning all DNF laps to ``No
    ...
    still running competitively before retiring. A more nuanced approach,
    such as using running position at the time of retirement, could improve
    model training.
\end{enumerate}
