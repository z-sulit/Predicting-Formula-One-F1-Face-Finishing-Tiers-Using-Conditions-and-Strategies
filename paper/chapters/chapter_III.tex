...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{figures/figmethod.pdf}
  \caption{Six-Stage Methodological Framework for Multivariate Predictive Modeling}
...
\begin{description}
  ...
  \item[Multinomial Logistic Regression] This was the primary technique
    selected for the study. Using Maximum Likelihood Estimation (MLE), it
    calculates the log-odds of a driver landing in each tier relative to a
    baseline category. It also offers greater operational value for risk
...
term. Following \textcite{hair2019}, the variables were \textit{not}
dropped. Instead, a dynamic interaction term was constructed:
...
The Multinomial Logistic Regression was estimated with Maximum Likelihood
...
\begin{aligned}
  ...
  &= \beta_{0k} + \beta_{1k} \cdot \text{Grid} + \beta_{2k} \cdot \text{TireAge} \\
  &\quad + \beta_{3k} \cdot \text{Fuel} + \beta_{4k} \cdot (\text{Deg} \times \text{Fuel}) \\
  &\quad + \beta_{5k} \cdot \text{Tire\_MED} + \beta_{6k} \cdot \text{Tire\_SOFT}
\end{aligned}
\]
...
\begin{itemize}
  \item \textbf{Python (pandas, scikit-learn):} Data wrangling, dummy
    ...
    Cross-validation used scikit-learn logistic regression with balanced
    class weights.
...
\begin{itemize}
  \item \textbf{Data transparency:} The full dataset and all source code
    ...
    \href{https://github.com/z-sulit/Predicting-Formula-One-F1-Face-Finishing-Tiers-Using-Conditions-and-Strategies}{project GitHub repository}
